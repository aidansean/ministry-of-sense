...
function spy_object(){
  ...
  self.start_dataTaking = function(){
    heartbeat() ;
    Get('button_dataTaking').style.display = 'none' ;
    Get('button_freeze'    ).style.display = ''     ;
  }
  ...
  self.write_final_result = function(){
    ...
    th.colSpan = 2 ;
    if(results_mode=='sigma'){
      th.innerHTML = spy.nSigma.toPrecision(2) + 'σ discovery!<br />A Nobel Prize awaits!' ;
    }
    else{
  ...
  self.pointless_run = function(){
    ...
    self.draw_pointless() ;
    window.setTimeout(self.sound_pointless, 250) ;
  }
...
function experiment_object(name){
  ...
  self.thresholdBase =  8 ;
  self.thresholdHigh = 24 ;
  ...
  self.topos = [] ;
  self.playerNames = [] ;
  self.playerNamesPool = [] ;
  self.collision_id = -1 ;
  
  self.is_running_minimumBias = true ;
  self.nHeartbeatsSinceCollision = 0 ;
  ...
  self.receive_single_collision_from_server = function(){
    ...
    if(seed==-1){
      ...
      self.draw_collision_from_seed_and_topo(false) ;
    }
  }
  self.draw_collision_from_seed_and_topo = function(is_fake){
    ...
    var c = self.collision_context ;
    draw_eventDisplay(collision, c) ;
    
    c.save() ;
    
    if(playerName!=''){
      c.fillStyle = 'rgba(255,255,255,0.75)' ;
      c.fillRect(0,0.8*ch,cw,0.2*ch) ;
      
      c.strokeStyle = 'rgb(255,255,255)' ;
      c.fillStyle = 'rgb(0,0,0)' ;
      c.textAlign = 'center' ;
      c.textBaselie = 'middle' ;
      c.font = 0.15*ch + 'px arial' ;
      c.fillText  (playerName, 0.5*cw, 0.95*ch) ;
      c.strokeText(playerName, 0.5*cw, 0.95*ch) ;
      c.restore() ;
    }
    ...
    if(is_fake){
      ...
      c.lineWidth = 0.01*cw ;
      c.beginPath() ;
      c.arc(0.95*cw, 0.85*ch, 0.02*cw, 0, 2*pi, true) ;
      c.stroke() ;
      c.restore() ;
    }
  }
...
function activate_buttons(){
  Get('button_dataTaking'            ).addEventListener('click', spy.start_dataTaking        ) ;
...
function start(){
  ...
  var playerNames = ['Alan','ben','Chris','Donna','freddie','Gem x','John','Kelli','Lisa','Pete','rach <3','Stephen','Terri','Vi'] ;
  while(playerNames.length>0){
    var n = playerNames.length ;
    var index = floor(random()*n) ;
    if(n%2==0){
      experiments['ATLAS'].playerNamesPool.push(playerNames.splice(index,1)) ;
    }
    else{
      experiments['CMS'  ].playerNamesPool.push(playerNames.splice(index,1)) ;
    }
  }
  ...
  Get('button_pointless_noFinishLine').style.background = colorBoth ;
...
function heartbeat(){
  ...
  for(var i=0 ; i<team_names.length ; i++){
    ...
    var callback = experiments[team_names[i]].receive_single_collision_from_server ;
    ex.xmlhttp.onreadystatechange = callback ;
    ex.xmlhttp.send(null) ;
    ex.xmlhttp_muted = true ;
    
    // Fake results if nothing happens in time.
    ex.nHeartbeatsSinceCollision++ ;
    var threshold = floor(ex.thresholdBase + random()*ex.thresholdHigh-ex.thresholdBase) ;
    if(ex.nHeartbeatsSinceCollision>threshold){
      ex.nHeartbeatsSinceCollision = 0 ;
      var topologies = [ ['electron'] , ['muon'] , ['electron','electron'] , ['muon','muon'] , ['electron','muon'] ] ;
      var playerNames = ex.playerNames ;
      var topology   =  topologies[floor(random()*topologies .length)] ;
      var playerName = ex.playerNamesPool[floor(random()*ex.playerNamesPool.length)] ;
      var coll       = make_matching_collision(topology) ;
      
      ex.seeds.push(coll.seed) ;
      ex.topos.push(topology) ;
      ex.playerNames.push(playerName) ;
      
      ex.draw_collision_from_seed_and_topo(true) ;
    }
  }
...
function heartbeat_fast(){
  ...
  var g = Math.floor(150 + pulse*105/255.0) ;
  var color = 'rgb(' + c + ',' + g + ',' + c + ')'  ;
